...
function PPTVParser(options) {
  this.options = options;
}


PPTVParser.prototype.parseId = function(id, callback) {
  var self = this;

  request({
    url: util.format('http://web-play.pptv.com/webplay3-0-%s.xml?zone=8&pid=5701&username=&salt=pv&o=0&referer=&param=type%3Dweb.fpp%26userType%3D0%26o%3D0&version=4&type=web.fpp&r=%d&pageUrl=%s', id, Date.now() / 1000000 >> 0, this.referer_url),
    proxy: self.options.proxy
  }, function(err, resp, body) {
    // var host = body.match(/<sh>([^<>]+)<\/sh>/);
    if (err || resp.statusCode != 200) {
      callback && callback(err || new Error('get pptv info failed'));
      return;
    }
...
PPTVParser.prototype.parse = function(url, callback) {
  var self = this;
  ...
  request({
    ...
    if (err || resp.statusCode != 200) {
      callback && callback(err || new Error('get pptv page failed'));
      return;
    }
    ...
    self.parseId(cid, callback);
  });
};

module.exports = PPTVParser;
