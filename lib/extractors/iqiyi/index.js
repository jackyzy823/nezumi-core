...
function getVMS(tvid, vid, enckey, qyid, callback) {
  var tm = (Math.random() * 500 >> 0 + 500).toString();
  ...
  request.get({
    ...
    if (err || resp.statusCode != 200) {
      callback && callback(new Error('iqiyi get vms error'));
      return;
    }
...
function getEncKey(swflink, callback) {
  request.get({
    ...
    if (err || resp.statusCode != 200) {
      callback && callback(new Error('get swf file error'));
      return;
    }
...
function parse(url, options, callback) {
  ...
  request.get(url, function(err, resp, body) {
    if (err || resp.statusCode != 200) {
      callback && callback(new Error('get page error'));
      return;
    }
    ...
    async.waterfall([
      function firstTry(callback) {
        getVMS(tvid, videoid, ENCRYPT_KEY, qyid, function(err, info) {
          if (err) {
            callback && callback(null, true, null);
            return;
          }
          callback && callback(null, false, info);
        });
      },
      function secondTry(needRetry, info, callback) {
        if (needRetry) {
          getEncKey(swflink, function(err, enckey) {
            if (err) {
              callback && callback(err);
              return;
            }
            getVMS(tvid, videoid, enckey, qyid, function(err, info) {
      ...
      function getInfo(info, callback) {
        ...
        if (info.data.vp.tkl == '') {
          callback && callback(new Error('not support iqiyi VIP video'));
          return;
        }
        ...
        async.map(video_links, function(item, callback) {
          ...
          request.get({
            ...
            if (err || resp.statusCode != 200) {
              callback && callback(new Error('get real video link error'));
              return;
            }
...
if (module == require.main) {
  ...
  //   console.assert(result == ENCRYPT_KEY, 'Ooops!iqiyi changes it\'s enc key , a issue is welcomed!');
  // });
  parse('http://www.iqiyi.com/v_19rrnscu0w.html', null, function(err, result) {
