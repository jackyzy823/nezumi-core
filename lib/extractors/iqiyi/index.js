...
IQiyiParser.prototype.getVMS = function(tvid, vid, enckey, qyid, callback) {
  ...
  request.get({
    ...
    async.waterfall([
      function firstTry(callback) {
        self.getVMS(tvid, videoid, ENCRYPT_KEY, qyid, function(err, info) {
          if (err) {
      ...
      function secondTry(needRetry, info, callback) {
        if (needRetry) {
          self.getEncKey(swflink, function(err, enckey) {
            if (err) {
              callback && callback(err);
              return;
            }
            self.getVMS(tvid, videoid, enckey, qyid, function(err, info) {
