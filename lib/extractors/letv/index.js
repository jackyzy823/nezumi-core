...
LetvParser.prototype.getVid = function(url, callback) {
  request({
    ...
    if (err || resp.statusCode != 200) {
      callback(err || new Error('letv getvid error'));
      return;
    }
...
LetvParser.prototype.getVideoInfo = function(vid, callback) {
  ...
  request.get({
      ...
      if (err || resp.statusCode != 200) {
        callback && callback(err || new Error('get video info error'));
        return;
      }
      ...
      request.get({
        ...
        if (err || resp.statusCode != 200) {
          callback && callback(err || new Error('get m3u8 location error'));
          return;
        }
        request.get({
          url: body.location,
          ...
          if (err || resp.statusCode != 200) {
            callback && callback(err || new Error('get m3u8 info error'));
            return;
          }
...
LetvParser.prototype.parse = function(url, callback) {
  var self = this;
