  ...
  async.waterfall([function getPage(cb) {
    request.get({
      ...
      if (err || resp.statusCode != 200) {
        cb && cb(err || new Error('get sohu page failed'));
        return;
      }
    ...
    if (isMyTV) {
      request.get({
        ...
        if (err || resp.statusCode != 200) {
          cb && cb(err || new Error('get sohu tv info failed'));
          return;
        }
        var data = info.data;
        data.clipsBytes = data.clipsBytes.map(Number);
        cb && cb(null, vid, {
          host: info.allot,
          data: data
        });
      ...
      request.get({
        ...
        if (err || resp.statusCode != 200) {
          cb && cb(err || new Error('get sohu my tv info failed'));
          return;
        }
        if (!info.data) {
          cb && cb(new Error('sohu content region restricted!please use proxy!'));
          return;
        }
        var hqvid = null;
        // TODO add quality_choice
        ...
        if (ret) {
          request.get({
            ...
            if (err || resp.statusCode != 200) {
              cb && cb(err || new Error('get sohu hq tv info failed'));
              return;
            }
            var data = info.data;
            data.clipsBytes = data.clipsBytes.map(Number);
    ...
    async.map(zipdata, function(item, cb) {
      ...
      request.get({
        ...
        if (err || resp.statusCode != 200) {
          cb && cb(err || new Error('get sohu url failed'));
          return;
        }
